...
var MExpense = {
    ...
    // Guardar
    save: function(){
        api('expenses.php', MExpense.$form.serializeObject(), function(rsp){
            if(rsp.ok == true){
    ...
    remove: function(id){
        bootbox.confirm('¿Realmente desea eliminar?', function(result){
            if(result){
                api('expenses.php', {action:'remove', id:id}, function(rsp){
                    if(rsp.ok == true){
    ...
    setPaid: function(id){
        bootbox.confirm('¿Marcar como pagado?', function(result){
            if(result){
                api('expenses.php', {action:'set_paid', id:id}, function(rsp){
                    if(rsp.ok == true){
                        toastr.success('Guardado correctamente');
                        location.reload();
                    } else {
